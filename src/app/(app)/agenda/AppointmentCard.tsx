...
import { Calendar, Trash2, X, Check, Clock } from "lucide-react";
...
export function AppointmentCard({ appointment: a }: { appointment: any }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 p-2.5 hover:border-brand-300 hover:shadow-sm bg-white transition-all relative group">
      <Link href={`/atendimento/${a.id}`} className="block">
        <div className="flex items-center justify-between gap-1.5">
          ...
          {a.statusRelation ? (
            <span className={cn(
              "px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase border tracking-wider",
              COLOR_CLASSES[a.statusRelation.color] || "bg-slate-100 text-slate-800 border-slate-200"
            )}>
              {a.statusRelation.name}
            </span>
          ) : (
            <span className="badge-gray text-[9px] font-semibold uppercase">
              {a.status.replace(/_/g, " ").toLowerCase()}
            </span>
          )}
        </div>
        <div className="text-sm font-bold text-slate-800 mt-1.5">{a.pet?.name ?? "Sem pet"}</div>
        <div className="text-xs text-slate-500 font-medium">{a.tutor.name}</div>
